...
/** This is to define the universal function in terms of bayes rule.
Its a universal lambda function (combinator) and pattern calculus function, that always curries 7 params.
A node is 1 of: universalFunction, doesNotHalt, or pair of node.
universalFunction and doesNotHalt are the 2 leafs, which all possible paths in the binary forest lead to,
just a data structure related by bayes math.
Every binary forest node is a bayes node.
Bayes weights are between the conditional probability of bayes nodes.
This uses only callpair math, though its compatible with callquad math.
<br><br>
# bayeslambda
...
<br><br>
8 opcodes, which all curry exactly 7 params (if they mention less than 7, then its λa.λb.λc.λd... before that)_, and which of 8 is chosen by the first 3 of 7 curried params each being leaf or anything except leaf, and param_4_of_7 is comment/ignoredExceptByLAndR:
<br><br>
S = λx.λy.λz.xz(yz)
<br><br>
T =  λx.λy.x
