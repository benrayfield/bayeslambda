...
/** immutable */
public final class Callquad{

	/** 1 of 2 leafs which all paths lead to 1 of */
	public static final Callquad universalFunction = new Callquad(true);
	
	/** 1 of 2 leafs which all paths lead to 1 of */
	public static final Callquad doesNotHalt = new Callquad(false);
	
	/** convenient shorter name for universalFunction */
	public static final Callquad u = universalFunction;
	
	/** convenient shorter name for doesNotHalt */
	public static final Callquad o = doesNotHalt;
	
	public final Callquad func, param, stackFunc, stackParam, cacheKey;
	...
	*/
	...
	private Callquad(boolean isUniversalFunction){
	...
	private Callquad(boolean isDone, boolean isForceDeterminismRecursively, Callquad func, Callquad param, Callquad stackFunc, Callquad stackParam, Callquad cacheKey){
		this.isDone = isDone;
	...
	public boolean equals(Object o){
		...
		if(!(o instanceof Callquad)) return false;
		Callquad n = (Callquad)o;
	...
	private static final Map<Callquad,Callquad> dedup = new HashMap();
	
	/** deduped forest node */
	private static final Callquad node(boolean isHalted, boolean isForceDeterminismRecursively,
			Callquad func, Callquad param, Callquad stackFunc, Callquad stackParam, Callquad cacheKey){
		Callquad n = new Callquad(isHalted, isForceDeterminismRecursively, func, param, stackFunc, stackParam, cacheKey);
		Callquad d = dedup.get(n);
		if(d == null){
			d = n;
			dedup.put(d, d);
		}
	...
	*/
	public static final Callquad op(int zeroToSeven){
	...
	/** λx.λy.λz.xz(yz) */
	public static final Callquad S = op(0);
	
	/** λx.λy.x */
	public static final Callquad T = op(1).hcp(u);
	
	/** λx.λy.y */
	public static final Callquad F = op(2).hcp(u);
	
	/** λx.(pair (VM_L x) (VM_R x)) */
	public static final Callquad pairOfLROf = op(3).hcp(u).hcp(u);
	
	/** λx.{VM_isLeaf x, returns T or F} */
	public static final Callquad isLeaf = op(4).hcp(u).hcp(u);
	
	/** λx.λy.λz.zxy */
	public static final Callquad pair = op(5);
	...
	*/
	public static final Callquad curry = op(6).hcp(u);
	...
	*/
	public static final Callquad superposition = op(7);
	
	
	
	/** λy.y, aka identityFunction */
	public static final Callquad I = F.hcp(u);
	...
	*/
	public static final Callquad typeval = setComment(pair,uu);
	//public static final Callquad typeval = T.func().hcp(uu);
	
	/** TODO create a Callquad which does this, instead of hardcoding it in java, but also use the java form as an optimization sometimes. */
	public static int cur(Callquad n){
	...
	TODO create a Callquad which does this, instead of hardcoding it in java.
	*/
	public static Callquad setComment(Callquad n, Callquad comment){
		/*int cur = cur(n);
		if(cur == 3) return 
	...
	public static void main(String[] args){
		...
		Callquad leafLeaf = universalFunction.cp(universalFunction);
		Callquad leafLeaf_leaf = leafLeaf.cp(universalFunction);
		Callquad leaf_leafLeaf = universalFunction.cp(leafLeaf);
		Callquad leafLeaf_leafLeaf = leafLeaf.cp(leafLeaf);
		Callquad leaf_leafLeaf_again = universalFunction.cp(universalFunction.cp(universalFunction));
		if(leaf_leafLeaf != leaf_leafLeaf_again) throw new Error("Didnt dedup");
		Callquad leafLeaf_leafLeaf_again = leafLeaf.cp(leafLeaf);
		...
		Callquad leafNonhalt = universalFunction.cp(doesNotHalt);
		Callquad leafNonhalt_leaf = leafNonhalt.cp(universalFunction);
		Callquad leafNonhalt_again = universalFunction.cp(doesNotHalt);
		Callquad leafNonhalt_leaf_again = leafNonhalt_again.cp(universalFunction);
